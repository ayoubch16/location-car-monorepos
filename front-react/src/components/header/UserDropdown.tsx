...
export function UserDropdown() {
  ...
  const displayName = user ? `${user.nom} ${user.prenom}` : "User";
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11 bg-brand-100 dark:bg-brand-900 flex items-center justify-center text-brand-600 dark:text-brand-300 font-semibold text-sm">
          {user ? (`${user.nom?.[0] ?? ""}${user.prenom?.[0] ?? ""}`).toUpperCase() || "U" : "U"}
        </span>
